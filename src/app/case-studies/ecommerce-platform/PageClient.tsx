  ...
  return (
    <>
      ...
      <main ref={pageRef as React.RefObject<HTMLElement>} style={{ background: 'var(--surface)', minHeight: '100vh' }}>
        ...
        <section style={{ padding: 'clamp(100px, 12vw, 140px) 0 clamp(40px, 5vw, 64px)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="cb-container">
            ...
            <figure className="media media--lg fade" style={{ aspectRatio: '16 / 9', margin: '0 0 48px' }}>
              <Image src={caseStudyMedia['ecommerce-platform'].src} alt={caseStudyMedia['ecommerce-platform'].alt} fill priority sizes="(max-width: 1400px) 100vw, 1320px" style={{ objectFit: 'cover' }} />
            </figure>
          </div>
        </section>
        ...
        <section style={{ paddingBottom: 'var(--section-y)', borderTop: '1px solid var(--line)' }}>
          <div className="cb-container" style={{ paddingTop: 'var(--section-y)' }}>
            <div className="reveal" style={{
              ...
              {[
                { title: 'Challenge', text: 'An Edmonton-based multi-vendor retailer was stuck on a legacy monolithic platform that could not handle traffic spikes, had a 6-second average page load, and required weeks of developer time for simple catalog changes.' },
                { title: 'Solution', text: 'Mapletech Labs rebuilt the entire platform as a headless commerce system using Next.js, Stripe for payments, Elasticsearch for product search, and a custom vendor management portal — delivered in 5 months.' },
        ...
        <MediaBand media={humanPick('ecommerce-platform')} ratio="21 / 8" />
