  ...
  return (
    <>
      ...
      <main style={{ background: 'var(--surface)', color: 'var(--ink)', fontFamily: 'inherit' }}>
        ...
        <section ref={heroRef} style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', paddingTop: 'var(--section-y)', paddingBottom: 'var(--section-y)' }}>
          <div className="cb-container">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 480px), 1fr))', gap: 'clamp(24px, 5vw, 60px)', alignItems: 'center' }}>
              <div>
                <div className="reveal" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 28, fontSize: 12, color: 'var(--muted)' }}>
                  <Link href="/" style={{ color: 'var(--muted)', textDecoration: 'none' }}>Home</Link>
                  <span>/</span>
                  <Link href="/services/digital-marketing" style={{ color: 'var(--muted)', textDecoration: 'none' }}>Digital Marketing</Link>
                  <span>/</span>
                  <span style={{ color: 'var(--brand)' }}>SEO Services</span>
                </div>
                <div className="reveal reveal-d1" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 100, background: 'rgba(245,41,13,0.08)', border: '1px solid rgba(245,41,13,0.2)', marginBottom: 24 }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--brand)', display: 'inline-block' }} />
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--brand)', letterSpacing: '0.05em' }}>DIGITAL MARKETING</span>
                </div>
                <h1 className="reveal reveal-d2" style={{ fontSize: 'var(--fs-display)', fontWeight: 600, letterSpacing: '-0.04em', lineHeight: 1.05, marginBottom: 24, maxWidth: 800 }}>
                  SEO That Drives <span style={{ color: 'var(--brand)' }}>Real Growth</span>
                </h1>
                <p className="reveal reveal-d3" style={{ fontSize: 18, color: 'var(--muted)', lineHeight: 1.7, maxWidth: 600, marginBottom: 40 }}>Data-driven search engine optimisation that moves you up the rankings, grows organic traffic, and converts visitors into customers — month after month.</p>
                <div className="reveal reveal-d4" style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  ...
                  <Link href="/case-studies" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: 52, padding: '0 32px', borderRadius: 100, border: '1px solid var(--line-strong)', color: 'var(--ink)', fontSize: 15, fontWeight: 600, textDecoration: 'none' }}>View Case Studies</Link>
                </div>
                <div className="reveal" style={{ display: 'flex', gap: 48, marginTop: 64, flexWrap: 'wrap' }}>
                  {stats.map(s => (
                    <div key={s.label}>
                      <div style={{ fontSize: 32, fontWeight: 800, color: 'var(--brand)', letterSpacing: '-0.03em' }}>{s.value}</div>
        ...
        <MediaBand
          media={serviceMedia['digital-marketing'] ?? defaultMedia}
        />
        <section ref={s1} style={{ padding: 'var(--section-y) 0' }}>
          <div className="cb-container">
            <div className="reveal" style={{ textAlign: 'center', marginBottom: 64 }}>
              ...
              <h2 style={{ fontSize: 'var(--fs-h2)', fontWeight: 600, letterSpacing: '-0.03em' }}>Our SEO Capabilities</h2>
            </div>
        ...
        <section ref={s3} style={{ padding: 'var(--section-y) 0' }}>
          <div className="cb-container" style={{ maxWidth: 800 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {faqs.map((faq, i) => (
                <div key={i} className="reveal" style={{ borderRadius: 16, border: '1px solid var(--line)', overflow: 'hidden' }}>
                  <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ width: '100%', padding: '24px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', gap: 16 }}>
                    <span style={{ fontSize: 16, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.01em' }}>{faq.q}</span>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f5290d" strokeWidth="2" style={{ flexShrink: 0, transition: '0.3s', transform: openFaq === i ? 'rotate(45deg)' : 'none' }}><path d="M12 5v14M5 12h14" /></svg>
                  </button>
                  {openFaq === i && (
                    <div style={{ padding: '0 28px 24px', fontSize: 15, color: 'var(--muted)', lineHeight: 1.75 }}>{faq.a}</div>
                  )}
                </div>
              ))}
            </div>
        ...
        <section ref={s4} style={{ padding: 'var(--section-y) 0', textAlign: 'center' }}>
          <div className="cb-container">
            <h2 className="reveal" style={{ fontSize: 'var(--fs-h2)', fontWeight: 600, letterSpacing: '-0.04em', marginBottom: 20 }}>Ready to Rank Higher?</h2>
            <p className="reveal reveal-d1" style={{ fontSize: 18, color: 'var(--muted)', marginBottom: 40, maxWidth: 500, margin: '0 auto 40px' }}>Let's build an SEO strategy that drives sustainable organic growth for your business.</p>
