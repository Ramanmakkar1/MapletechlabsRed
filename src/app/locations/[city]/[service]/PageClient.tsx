...
function hoverCard(e: React.MouseEvent, on: boolean) {
  ...
  t.style.borderColor = on ? 'rgba(245,41,13,0.2)' : 'var(--line)';
  t.style.background = on ? 'rgba(245,41,13,0.03)' : 'rgba(20,17,24,0.035)';
  t.style.transform = on ? 'translateY(-4px)' : '';
  t.style.boxShadow = on ? '0 24px 60px rgba(20,17,24,0.10)' : '';
}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 480px), 1fr))', gap: 'clamp(24px, 5vw, 80px)', alignItems: 'center' }}>
              ...
              <div>
                ...
                <nav className="reveal" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
                  {[
                    ...
                    <span key={crumb.label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <Link href={crumb.href} style={{ fontSize: 13, color: 'var(--muted)', textDecoration: 'none', transition: 'color 0.2s' }}
                  ...
                  <span style={{ fontSize: 13, color: 'var(--body)' }}>{serviceName}</span>
                </nav>
                ...
                <div className="reveal" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: 'rgba(245,41,13,0.08)', border: '1px solid rgba(245,41,13,0.2)', borderRadius: 100, padding: '8px 20px', marginBottom: 32 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--brand)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>{badge}</span>
                </div>
                ...
                <div className="reveal reveal-d3" style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 48 }}>
                  <Link href="/contact" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, height: 56, padding: '0 36px', borderRadius: 100, background: 'var(--brand)', color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none', transition: '0.3s' }}
                  ...
                  <Link href="/case-studies" style={{ display: 'inline-flex', alignItems: 'center', height: 56, padding: '0 36px', borderRadius: 100, border: '1px solid var(--line)', color: 'var(--ink)', fontSize: 14, fontWeight: 500, textDecoration: 'none', transition: '0.3s' }}
                ...
                <div className="reveal reveal-d4 loc-hero-stats" style={{ display: 'grid', gridTemplateColumns: `repeat(${stats.length}, 1fr)`, gap: 16 }}>
                  {stats.map((s, i) => (
                    <div key={s.label} style={{ textAlign: 'center', borderRight: i < stats.length - 1 ? '1px solid var(--line)' : 'none', paddingRight: i < stats.length - 1 ? 16 : 0 }}>
                      <div style={{ fontSize: 'clamp(1.4rem, 2.5vw, 2rem)', fontWeight: 700, color: 'var(--brand)', letterSpacing: '-0.03em' }}>{s.value}</div>
                      <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--faint)', textTransform: 'uppercase', letterSpacing: '0.08em', marginTop: 4 }}>{s.label}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <section ref={statsRef} style={{ ...sectionBorder }}>
          <div className="cb-container">
            <div className="reveal loc-stats-strip" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
              {stats.map((s, i) => (
                <div key={s.label} style={{ padding: '52px 40px', borderRight: i < 3 ? '1px solid var(--line)' : 'none', textAlign: 'center' }}>
                ...
                <div key={s.title} className="loc-large-card" style={{ ...cardStyle, padding: '48px 44px', borderRadius: 32, position: 'relative', overflow: 'hidden' }}
                  ...
                  {s.tags && s.tags.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                      {s.tags.map(tag => (
                        <span key={tag} style={{ fontSize: 12, fontWeight: 600, color: 'var(--muted)', padding: '6px 14px', border: '1px solid var(--line)', borderRadius: 100 }}>{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
            ...
            <div className="reveal reveal-d2" style={autoGrid}>
              {smallServices.map(s => (
                <div key={s.title} style={{ ...cardStyle, padding: '32px 28px', borderRadius: 24, position: 'relative', overflow: 'hidden' }}
                  ...
                  <h3 style={{ fontSize: 17, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.02em', marginBottom: 10 }}>{s.title}</h3>
            ...
            <div className="reveal reveal-d1" style={autoGrid}>
              {whyCity.map((w, i) => (
                <div key={w.title} className={`reveal reveal-d${Math.min(i + 1, 4)} loc-why-card`} style={{ ...cardStyle, padding: '44px 36px' }}
                  ...
                  <h3 style={{ fontSize: 20, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.02em', marginBottom: 12 }}>{w.title}</h3>
            ...
            <div className="reveal reveal-d1" style={autoGrid}>
              {applications.map((a, i) => (
                <div key={a.industry} className={`reveal reveal-d${Math.min(i + 1, 4)}`} style={{ ...cardStyle, padding: '36px 32px' }}
                  ...
                  <h3 style={{ fontSize: 19, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.02em', marginBottom: 12, lineHeight: 1.3 }}>{a.title}</h3>
                  <p style={{ fontSize: 15, color: 'var(--muted)', lineHeight: 1.75, margin: 0 }}>{a.desc}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {steps.map((step, i) => (
                  <div key={step.num} className={`reveal reveal-d${Math.min(i + 1, 5)} loc-process-row`} style={{ display: 'grid', gridTemplateColumns: '48px 1fr', gap: 32, alignItems: 'start', padding: '32px 0' }}>
                    <div className="loc-process-num" style={{ width: 48, height: 48, borderRadius: '50%', border: '2px solid rgba(245,41,13,0.4)', background: 'rgba(245,41,13,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: 'var(--brand)', flexShrink: 0, position: 'relative', zIndex: 1 }}>{step.num}</div>

                    <div className="loc-process-card" style={{ ...cardStyle, padding: '32px 40px' }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(245,41,13,0.2)'; (e.currentTarget as HTMLElement).style.background = 'rgba(245,41,13,0.03)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 20px 50px rgba(20,17,24,0.10)'; }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--line)'; (e.currentTarget as HTMLElement).style.background = 'var(--surface-alt)'; (e.currentTarget as HTMLElement).style.boxShadow = ''; }}>
            ...
            <div className="reveal reveal-d1" style={autoGrid}>
              {techCategories.map(cat => (
                <div key={cat.title} style={{ ...cardStyle }}
                  ...
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--line)'; (e.currentTarget as HTMLElement).style.background = 'var(--surface-alt)'; }}>
                  <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--brand)', marginBottom: 20 }}>{cat.title}</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {cat.items.map(item => (
                      <span key={item} style={{ padding: '7px 16px', border: '1px solid var(--line)', borderRadius: 100, fontSize: 13, fontWeight: 500, color: 'var(--muted)', transition: '0.25s', cursor: 'default' }}
        ...
        <section ref={portfolioRef} style={{ ...sectionPad, ...sectionBorder }}>
          <div className="cb-container">
            <div className="reveal" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 64, gap: 40, flexWrap: 'wrap' }}>
              ...
              <Link href="/case-studies" style={{ fontSize: 14, fontWeight: 600, color: 'var(--brand)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8, transition: '0.2s' }}
              ...
              {portfolioItems.map((p, i) => (
                <div key={p.title} className={`reveal reveal-d${i + 1}`} style={{ ...cardStyle, padding: 0, overflow: 'hidden', borderRadius: 28 }}
                  ...
                  <div style={{ height: 200, background: p.gradient, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ width: 60, height: 60, borderRadius: 16, background: 'var(--surface-sunk)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M3 9h18M9 21V9" /></svg>
                    </div>
                  </div>
                  <div className="loc-portfolio-inner" style={{ padding: '28px 32px' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--brand)', marginBottom: 10, display: 'block' }}>{p.industry}</span>
                    <h3 style={{ fontSize: 18, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.02em', marginBottom: 10 }}>{p.title}</h3>
                    <p style={{ fontSize: 14, color: 'var(--muted)', lineHeight: 1.7, marginBottom: 16 }}>{p.desc}</p>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: 'var(--brand)', background: 'rgba(245,41,13,0.08)', padding: '6px 14px', borderRadius: 100 }}>
              ...
              {testimonials.map((t, i) => (
                <div key={t.name} className={`reveal reveal-d${Math.min(i + 1, 3)} loc-testimonial-card`} style={{ ...cardStyle, padding: '44px 36px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 16, color: 'var(--body)', lineHeight: 1.8, fontStyle: 'italic', marginBottom: 28 }}>
                      &ldquo;{t.quote}&rdquo;
                    </p>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(245,41,13,0.12)', border: '1px solid rgba(245,41,13,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: 'var(--brand)', flexShrink: 0 }}>
                      {t.name.split(' ').map(n => n[0]).join('')}
                    </div>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--ink)' }}>{t.name}</div>
                      <div style={{ fontSize: 12, color: 'var(--muted)' }}>{t.role}, {t.company}</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <section ref={pricingRef} style={{ ...sectionPad, ...sectionBorder }}>
          <div className="cb-container">
            <div className="reveal" style={{ textAlign: 'center', marginBottom: 64 }}>
              ...
              <p style={{ fontSize: 16, color: 'var(--muted)', marginTop: 16, maxWidth: 560, margin: '16px auto 0' }}>
            ...
            <div className="reveal reveal-d1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 20 }}>
              {pricingTiers.map((tier, i) => (
                <div key={tier.name} className={`reveal reveal-d${Math.min(i + 1, 3)} loc-pricing-card`}
                  ...
                  onMouseEnter={e => hoverCard(e, true)} onMouseLeave={e => hoverCard(e, false)}>
                  {i === 1 && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: 'var(--brand)' }} />}
                  <div style={{ marginBottom: 24 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: i === 1 ? 'var(--brand)' : 'var(--line-strong)', marginBottom: 12 }}>{tier.name}</div>
                    <div style={{ fontSize: 'clamp(1.8rem, 3vw, 2.6rem)', fontWeight: 700, color: 'var(--ink)', letterSpacing: '-0.03em', lineHeight: 1.1 }}>{tier.range}</div>
                  </div>
                  <div style={{ flex: 1, marginBottom: 32 }}>
                    {tier.features.map(f => (
                      <div key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 14 }}>
                        ...
                        <span style={{ fontSize: 14, color: 'var(--body)', lineHeight: 1.5 }}>{f}</span>
                      </div>
                    ))}
                  </div>
                  <Link href="/contact" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '16px 36px', borderRadius: 100, background: i === 1 ? '#f5290d' : 'transparent', border: i === 1 ? 'none' : '1px solid var(--line-strong)', color: 'var(--ink)', fontSize: 14, fontWeight: 700, textDecoration: 'none', transition: '0.3s' }}
        ...
        <section ref={faqRef} style={{ ...sectionPad, ...sectionBorder }}>
          <div className="cb-container" style={{ maxWidth: 860 }}>
            ...
            <div className="reveal reveal-d1" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {faqs.map((faq, i) => (
                <div key={i}
                  style={{ background: openFaq === i ? 'rgba(245,41,13,0.04)' : 'rgba(20,17,24,0.035)', border: `1px solid ${openFaq === i ? 'rgba(245,41,13,0.2)' : 'var(--line)'}`, borderRadius: 20, overflow: 'hidden', transition: 'border-color 0.3s, background 0.3s' }}>
                  ...
                  <div
                    ...
                    <p className="loc-faq-answer" style={{ padding: '0 28px 24px', margin: 0, fontSize: 15, color: 'var(--muted)', lineHeight: 1.75 }}>{faq.a}</p>
                  </div>
                </div>
              ))}
            </div>
                  ...
                  {relatedCityServices.map(rs => (
                    <Link key={`${rs.citySlug}-${rs.serviceSlug}`} href={`/locations/${rs.citySlug}/${rs.serviceSlug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                      <div style={{ ...cardStyle, padding: '24px 28px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
                        onMouseEnter={e => hoverCard(e, true)} onMouseLeave={e => hoverCard(e, false)}>
                        <span style={{ fontSize: 15, fontWeight: 500, color: 'var(--ink)' }}>{rs.name}</span>
                ...
                <div style={autoGrid}>
                  {sameServiceOtherCities.map(oc => (
                    <Link key={oc.citySlug} href={`/locations/${oc.citySlug}/${oc.serviceSlug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                      <div style={{ ...cardStyle, padding: '24px 28px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
                        onMouseEnter={e => hoverCard(e, true)} onMouseLeave={e => hoverCard(e, false)}>
                        <span style={{ fontSize: 15, fontWeight: 500, color: 'var(--ink)' }}>{serviceName} in {oc.cityName}</span>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#f5290d" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
                      </div>
                    </Link>
                  ))}
        ...
        <section style={{ padding: 'var(--section-y) 0', position: 'relative', overflow: 'hidden', textAlign: 'center', ...sectionBorder }}>
          ...
          <div className="cb-container" style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 48 }}>
              ...
              <Link href={`/locations/${citySlug}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, height: 60, padding: '0 40px', borderRadius: 100, border: '1px solid var(--line)', color: 'var(--ink)', fontSize: 15, fontWeight: 500, textDecoration: 'none', transition: '0.3s' }}
                onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--line-strong)'}
                onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--line)'}>
