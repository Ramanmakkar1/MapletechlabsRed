...
export default function FAQSection() {
  ...
  return (
    <section ref={ref} id="faq" className="section-padding" style={{ background: 'var(--surface-alt)', borderTop: '1px solid var(--line)' }}>
      <div className="cb-container">
        <div className="faq-grid">
          <div className="reveal faq-sticky" style={{ position: 'sticky', top: 80 }}>
            ...
            <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3.5rem)', fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.04em', lineHeight: 1.05, marginBottom: 20 }}>
              Questions<br /><span style={{ color: 'var(--faint)' }}>Answered.</span>
            </h2>
            <p style={{ fontSize: 'clamp(14px, 2.5vw, 16px)', color: 'var(--body)', lineHeight: 1.7, marginBottom: 'clamp(24px, 5vw, 40px)' }}>
              Everything you need to know before starting a project with us.
            </p>
            <Link href="#contact" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, height: 52, padding: '0 28px', borderRadius: 100, background: 'var(--brand)', color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none', transition: '0.3s' }}
            ...
            {faqs.map((faq, i) => (
              <div key={i} style={{ borderBottom: '1px solid var(--line)', overflow: 'hidden' }}>
                ...
                <div style={{ maxHeight: active === i ? 500 : 0, overflow: 'hidden', transition: 'max-height 0.45s cubic-bezier(0.16,1,0.3,1)' }}>
                  <p style={{ fontSize: 'clamp(14px, 2.5vw, 15px)', color: 'var(--body)', lineHeight: 1.8, paddingBottom: 'clamp(16px, 3vw, 28px)', margin: 0 }}>{faq.a}</p>
                </div>
              </div>
            ))}
          </div>
