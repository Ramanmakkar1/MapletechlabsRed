...
export default function ContactPage() {
  ...
  const inp: React.CSSProperties = { width: '100%', padding: '15px 20px', fontSize: 16, fontFamily: 'inherit', background: 'var(--surface-alt)', border: '1px solid var(--line)', borderRadius: 14, color: 'var(--ink)', outline: 'none', transition: 'border 0.25s, box-shadow 0.25s', boxSizing: 'border-box' };
  const sel: React.CSSProperties = { ...inp, appearance: 'none' as const, backgroundImage: `url("data:image/svg+xml,%3Csvg width='12' height='7' viewBox='0 0 12 7' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1L6 6L11 1' stroke='var(--ink)' stroke-width='1.5' stroke-linecap='round'/%3E%3C/svg%3E")`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 18px center' };
  ...
  const fi = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => { e.currentTarget.style.borderColor = 'var(--brand)'; e.currentTarget.style.boxShadow = '0 0 0 3px transparent'; };
  ...
  return (
    <div style={{ background: 'var(--surface)', color: 'var(--ink)', paddingTop: 'var(--section-y)' }}>
      ...
      <section ref={heroRef} style={{ position: 'relative', overflow: 'hidden', padding: 'clamp(60px, 10vw, 100px) 0 clamp(60px, 10vw, 120px)', borderBottom: '1px solid var(--line)', textAlign: 'center' }}>
        ...
        {/* Rings */}
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 700, height: 700, borderRadius: '50%', border: '1px solid var(--brand)', pointerEvents: 'none', animation: 'spin-slow 30s linear infinite' }} />
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 500, height: 500, borderRadius: '50%', border: '1px solid var(--brand)', pointerEvents: 'none', animation: 'spin-slow 20s linear infinite reverse' }} />

        <div className="cb-container" style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28 }}>
          {/* Live badge */}
          <div className="reveal" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: 'transparent', border: '1px solid var(--brand)', borderRadius: 100, padding: '8px 20px' }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--brand)', boxShadow: '0 0 8px #E11900', animation: 'pulse-glow 2s infinite' }} />
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--brand)', letterSpacing: '-0.005em', textTransform: 'none' as const }}>Available for New Projects</span>
          </div>
          ...
          <p className="reveal reveal-d2" style={{ fontSize: 'clamp(1rem,1.8vw,1.2rem)', color: 'var(--body)', maxWidth: 540, lineHeight: 1.75, margin: 0 }}>
          ...
          <div className="reveal reveal-d3" style={{ display: 'flex', gap: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
            <a href="#contact-form" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 12, height: 56, padding: '0 clamp(24px, 4vw, 40px)', borderRadius: 100, background: 'var(--brand)', color: '#fff', fontSize: 15, fontWeight: 700, textDecoration: 'none', transition: '0.3s' }}
            ...
            <a href="mailto:hello@mapletechlabs.ca" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 56, padding: '0 clamp(24px, 4vw, 40px)', borderRadius: 100, border: '1px solid var(--line)', color: 'var(--ink)', fontSize: 15, fontWeight: 500, textDecoration: 'none', transition: '0.3s' }}
          ...
          <div className="reveal reveal-d4" style={{ display: 'flex', gap: 32, flexWrap: 'wrap', paddingTop: 'var(--section-y)', borderTop: '1px solid var(--line)', justifyContent: 'center', width: '100%', marginTop: 8 }}>
            {TRUST.map(t => (
              <div key={t.label} style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                ...
                <span style={{ fontSize: 13, color: 'var(--body)', fontWeight: 500, whiteSpace: 'nowrap' as const }}>{t.label}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="reveal reveal-d1 cp-engage-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(280px, 100%), 1fr))', gap: 20 }}>
            {ENGAGEMENT.map(o => (
              <a key={o.num} href="#contact-form"
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--brand)'; e.currentTarget.style.background = 'transparent'; e.currentTarget.style.transform = 'translateY(-6px)'; e.currentTarget.style.boxShadow = '0 32px 64px rgba(0,0,0,0.10)'; }}
                ...
                <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: '-0.005em', textTransform: 'none' as const, color: 'var(--brand)', background: 'transparent', padding: '5px 14px', borderRadius: 100, display: 'inline-block', alignSelf: 'flex-start' }}>{o.tag}</span>
                <div>
                  <h3 style={{ fontSize: 22, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.02em', marginBottom: 12 }}>{o.title}</h3>
                  <p style={{ fontSize: 15, color: 'var(--muted)', lineHeight: 1.7, margin: 0 }}>{o.desc}</p>
                </div>
                <div style={{ marginTop: 'auto', display: 'inline-flex', alignItems: 'center', gap: 8, color: 'var(--brand)', fontSize: 13, fontWeight: 700 }}>{o.cta}</div>
              </a>
            ))}
          </div>
        </div>
      ...
      <section id="contact-form" ref={formRef} style={{ padding: 'var(--section-y) 0', borderBottom: '1px solid var(--line)', scrollMarginTop: 80 }}>
        <div className="cb-container">
          <div className="reveal cp-contact-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: 'clamp(32px, 6vw, 80px)', alignItems: 'start' }}>
            ...
            {/* Left info col */}
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '-0.005em', textTransform: 'none' as const, color: 'var(--faint)', marginBottom: 20 }}>Get in Touch</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginBottom: 48 }}>
                {[
                  ...
                  <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--faint)', textTransform: 'none' as const, letterSpacing: '-0.005em', marginBottom: 3 }}>{item.label}</div>
                      {item.href
                        ? <a href={item.href} style={{ fontSize: 14, color: 'var(--brand)', fontWeight: 500, textDecoration: 'none' }}>{item.value}</a>
                        : <div style={{ fontSize: 14, color: 'var(--ink)', fontWeight: 500 }}>{item.value}</div>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* NDA note */}
              <div style={{ padding: '20px 22px', background: 'transparent', border: '1px solid var(--brand)', borderRadius: 16, display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                <span style={{ fontSize: 20, flexShrink: 0, marginTop: 2 }}>🔒</span>
                <span style={{ fontSize: 13, color: 'var(--body)', lineHeight: 1.65 }}>All enquiries are protected by NDA. Your ideas stay confidential from the very first conversation.</span>
              </div>
            </div>
            ...
            <div style={{ background: 'var(--surface-alt)', border: '1px solid var(--line)', borderRadius: 36, padding: 'clamp(20px, 4vw, 52px) clamp(20px, 4vw, 44px)' }}>
              {submitted ? (
                <div style={{ textAlign: 'center', padding: 'var(--section-y) 0' }}>
                  <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'transparent', border: '1px solid var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#E11900" strokeWidth="2"><path d="M20 6L9 17l-5-5" /></svg>
                  </div>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  <div style={{ marginBottom: 4 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '-0.005em', textTransform: 'none' as const, color: 'var(--faint)', marginBottom: 8 }}>Project Enquiry</div>
                    <h3 style={{ fontSize: 24, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.03em', margin: 0 }}>Start the Conversation</h3>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
                    <div>
                      ...
                      <select id="cp-projectType" name="projectType" value={form.projectType} onChange={handle} required aria-required="true" style={{ ...sel, color: form.projectType ? 'var(--ink)' : 'var(--line-strong)' }} onFocus={fi} onBlur={fo}>
                        <option value="" disabled>Select type</option>
                        {PROJECT_TYPES.map(t => <option key={t} value={t} style={{ background: 'var(--surface)', color: 'var(--ink)' }}>{t}</option>)}
                      </select>
                    </div>
                    <div>
                      <label htmlFor="cp-budget" style={lbl}>Budget Range</label>
                      <select id="cp-budget" name="budget" value={form.budget} onChange={handle} style={{ ...sel, color: form.budget ? 'var(--ink)' : 'var(--line-strong)' }} onFocus={fi} onBlur={fo}>
                        <option value="" disabled>Select range</option>
                        {BUDGETS.map(b => <option key={b} value={b} style={{ background: 'var(--surface)', color: 'var(--ink)' }}>{b}</option>)}
                      </select>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <select id="cp-timeline" name="timeline" value={form.timeline} onChange={handle} style={{ ...sel, color: form.timeline ? 'var(--ink)' : 'var(--line-strong)' }} onFocus={fi} onBlur={fo}>
                      <option value="" disabled>When do you need this?</option>
                      {TIMELINES.map(t => <option key={t} value={t} style={{ background: 'var(--surface)', color: 'var(--ink)' }}>{t}</option>)}
                    </select>
                  </div>
                  ...
                  {error && <div id="cp-form-error" role="alert" style={{ color: 'var(--brand)', fontSize: 13, fontWeight: 500, background: 'transparent', border: '1px solid var(--brand)', padding: '12px 16px', borderRadius: 12, marginTop: 4 }}>{error}</div>}

                  <button type="submit" disabled={isLoading} style={{ marginTop: 4, height: 60, borderRadius: 100, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, background: isLoading ? 'var(--surface-sunk)' : 'var(--brand)', color: isLoading ? 'var(--muted)' : '#fff', fontSize: 15, fontWeight: 700, cursor: isLoading ? 'not-allowed' : 'pointer', transition: '0.3s', fontFamily: 'inherit' }}
      ...
      <section ref={officesRef} style={{ padding: 'var(--section-y) 0', borderBottom: '1px solid var(--line)' }}>
        <div className="cb-container">
          ...
          <div className="reveal reveal-d1 cp-offices-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(240px, 100%), 1fr))', gap: 2 }}>
            {OFFICES.map((o, i) => (
              <div key={o.city}
                style={{ padding: 'clamp(28px, 4vw, 48px) clamp(24px, 3vw, 36px)', border: '1px solid var(--line)', borderRadius: 16, background: 'var(--surface-alt)', transition: '0.4s' }}
                onMouseEnter={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'var(--brand)'; e.currentTarget.style.zIndex = '1'; e.currentTarget.style.position = 'relative'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'var(--surface-alt)'; e.currentTarget.style.borderColor = 'var(--line)'; }}>
                <div style={{ width: 24, height: 3, background: 'var(--brand)', borderRadius: 2, marginBottom: 24 }} />
                <div style={{ fontSize: 28, marginBottom: 12 }}>{o.flag}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontSize: 18, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.02em' }}>{o.city}</span>
                  {o.hq && <span style={{ fontSize: 9, fontWeight: 700, color: 'var(--brand)', background: 'transparent', padding: '3px 8px', borderRadius: 100, letterSpacing: '0.08em' }}>HQ</span>}
                </div>
                <p style={{ fontSize: 13, color: 'var(--body)', lineHeight: 1.6, margin: '0 0 12px' }}>{o.address}</p>
                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--faint)', letterSpacing: '-0.005em', textTransform: 'none' as const }}>{o.tz}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section ref={faqRef} style={{ padding: 'var(--section-y) 0', borderBottom: '1px solid var(--line)' }}>
        <div className="cb-container" style={{ maxWidth: 860 }}>
          ...
          <div className="reveal reveal-d1" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {FAQS.map((faq, i) => (
              <div key={i} style={{ background: openFaq === i ? 'transparent' : 'rgba(0,0,0,0.035)', border: `1px solid ${openFaq === i ? 'transparent' : 'var(--line)'}`, borderRadius: 20, overflow: 'hidden', transition: 'border-color 0.3s, background 0.3s' }}>
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'clamp(16px, 3vw, 24px) clamp(16px, 3vw, 28px)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
                  ...
                  <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--surface-alt)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'transform 0.3s', transform: openFaq === i ? 'rotate(45deg)' : 'none' }}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={openFaq === i ? '#E11900' : 'var(--body)'} strokeWidth="2"><path d="M12 5v14M5 12h14" /></svg>
                  </div>
                </button>
                {openFaq === i && (
                  <p style={{ padding: '0 clamp(16px, 3vw, 28px) clamp(16px, 3vw, 24px)', margin: 0, fontSize: 15, color: 'var(--muted)', lineHeight: 1.75 }}>{faq.a}</p>
                )}
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: 'var(--section-y) 0', position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
        ...
        <div className="cb-container" style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '-0.005em', textTransform: 'none' as const, color: 'var(--faint)', marginBottom: 20 }}>Let&apos;s Talk</div>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 56, padding: '0 clamp(24px, 4vw, 40px)', borderRadius: 100, border: '1px solid var(--line)', color: 'var(--ink)', fontSize: 15, fontWeight: 500, textDecoration: 'none', transition: '0.3s' }}
              onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--line-strong)'}
              onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--line)'}>
              View Our Work
            </Link>
